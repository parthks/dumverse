...
import NewButton from "@/components/ui/newBotton";

export default function App() {
  const { walletAddressID, profileLoading, getGameProfiles, gameProfiles } =
    useAppStore();
  useProfile();
  ...
  useEffect(() => {
    const handleWalletLoaded = () => {
      console.log(
        `You are using the ${window.arweaveWallet.walletName} wallet.`
      );
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden">
      ...
      <div className="relative z-10 h-full w-full">
        {!playing && (
          <div className="absolute top-[450px] left-[1110px] transform -translate-x-1/2">
            <button
              className="button group inline-flex items-center gap-2 transform"
              onClick={() => {
                if (videoRef.current) {
  ...
  return (
    <form className="space-y-4 w-full m-16 flex flex-col gap-4 items-center">
      ...
      <div className="flex flex-col gap-2 items-center justify-center">
        ...
        <div className="absolute mb-7 w-1/2 left-[190px]">
        <NewButton
          disabled={loading}
