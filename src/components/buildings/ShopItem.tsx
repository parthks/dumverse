...
export default function ShopItem({
  ...
  const priceText = item.gold_price
    ? `${item.gold_price.toLocaleString()} g`
    : item.dumz_price
    ? `${item.dumz_price.toLocaleString()} $Dumz`
    : "--";
  ...
  return (
    ...
    >
      <div className="relative w-full flex flex-col justify-center items-center">
        {/* Item image */}
        <img
          src={ITEM_IMAGES[item.id as keyof typeof ITEM_IMAGES]}
          alt={item.name}
        ...
        <div className="w-full relative">
          ...
          <div className="absolute top-[8px] left-0 right-[5px] z-20 text-center">
            ...
            <p className="text-white text-[90%] flex items-center justify-center">
              {priceText}
              <img
                src={item.gold_price ? IMAGES.GOLD_ICON : IMAGES.DUMZ_ICON}
                alt="Currency"
      ...
      <ImgButton
        disabled={
          buyItemLoading || (alreadyOwned && ["WEAPON", "ARMOR"].includes(item.type))
        }
